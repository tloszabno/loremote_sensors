    ...
    def get_humid_reading(self):
        try:
            for i in range(1, NUMBER_OF_READS_BEFORE_SAVE_MEASUREMENT):
                Adafruit_DHT.read_retry(config.humid_sensor_type, config.humid_sensor_port)
            humidity, temperature = Adafruit_DHT.read_retry(config.humid_sensor_type, config.humid_sensor_port)
